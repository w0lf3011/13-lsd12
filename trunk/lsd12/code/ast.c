 ...
 * Commentaire : "ast.h"
 */
ASTTREE createNode(int id, int type, int varRef, int fnctId, int ival, char* sval, ASTTREE left, ASTTREE right) {
...
/*
 * Retourne un type de noeud compréhensible par l'homme.
 * Pré-condition : /
 * Post-condition: Convertit les identifiants des noeuds en un nom compréhensible.
 */
...
/*
 * Affiche un arbre suivant un certain format pour être utiliser par GraphViz.
 * GraphiViz : logiciel permettant de générer une image à partir d'un code,
 * utiliser ici pour générer la représentation de l'arbre.
 */
