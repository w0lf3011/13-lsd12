...
int check(ASTTREE tree, SYMTABLE tds)
{
	...
	if (tree == NULL) {
		...
		switch (tree->id) {
			...
			case AT_AFFECT : 
				...
				if (node != NULL) {	
					switch(node->varType) {
						...
						default :
							printf(";Erreur l'affectation a %s est mal typee\n",tree->left->sval);
							return KO;
					}
					return KO;
				}
			...
			case AT_IF : // Si "ExprD" est de type VAL_BOOL
			...
			case AT_WHILE :	// Si "ExprD" est de type VAL_BOOL
				if(checkType(tree->left,VAL_BOOL,tds) == 3) {
					if(tree->right == NULL)
						return OK;
					else
		...
		switch (tree->id) {
			...
			case AT_APPELF :
				...
				if(nbOverLoad != -1) {// si fonction existe
						...
						if(tree->right == NULL) {
							...
							if(function->down->state != 2) {
								if(type == function->varType) 
									...
									check = 1;
									cpt ++;
									continue;	// ERROR : La fonction retourne un type incorrect
								}
							}
							else {
								check = 1;
								cpt ++;
								continue;		//ERROR Appel de la fonction avec trop peu arguments
							}	
						}
						else {	// si appel de fonction AVEC arg
							...
							while(treeSafe != NULL) {
								if(node->state != 2 || node == NULL) {
									...
									cpt ++;
									b = 1;
									break;	//ERROR Appel de la fonction avec trop arguments
								}
								if(checkType(treeSafe->right,node->varType,tds) != node->varType) {
									check = 1;
									cpt ++;
									b = 1;
									break;	//ERROR Argument mal type dans l'appel de fonction 
								}
	
								node = node->next; // Argument suivant dans TDS
								treeSafe = treeSafe->left; // Argument suivant dans Arbre
							}
							if(b == 1) {	// Erreur donc on passe au la fonction suivante -> cpt +1
								continue;
							}		
					...
					else {	// Error à l'appel de la fonction
						return KO;
					}
				}
				else {
